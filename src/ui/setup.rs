...
impl ConfigState {
    ...
    fn add_threads(&mut self, buf: &mut Buffers, threads: i64) {
        let new_threads = (self.settings.threads as i64) + threads;
    ...
    fn add_hash_size(&mut self, buf: &mut Buffers, hash_size: i64) {
        let new_hash_size = (self.settings.hash_size as i64) + hash_size;
...
#[inline]
// fn draw_spinner(gl: &mut Gl, uic: &mut UiContext, id: u64, callback: |i64|) {
fn draw_spinner(gl: &mut Gl, uic: &mut UiContext, id: u64, callback: i64) {
    uic.button(id + 1)
        .right_from(id, 2.0)
