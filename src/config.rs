...
impl ProgramSettings {
    fn opts() -> Vec<<Options as Example>::OptGroup> {
        let tmp: Options;
        vec![
            tmp.optopt(
                "t",
            ...
            tmp.optflag("", "help", "Display this help."),
            tmp.optflag(
                "u",
                "dup-only",
...
pub fn parse_args(args: &[String]) -> ProgramSettings {
    ...
    let ref opts = Options::getopts(args, settings_opts.as_slice()).unwrap();
...
fn dir_arg(args: &Matches, arg: &str, default: Path) -> Path {
    ...
    assert!(
        dir.is_dir(),
        "Value passed to {} is not a directory: {}",
        arg,
        dir.display()
    );
...
fn uint_arg(args: &Matches, arg: &str, default: usize) -> usize {
    let val = args
        .opt_str(arg)
        .map_or(default, |arg_str| arg_str.parse::<usize>().unwrap());
...
fn pos_f32_arg(args: &Matches, arg: &str, default: f32) -> f32 {
    let val = args
        .opt_str(arg)
        .map_or(default, |arg_str| arg_str.parse::<f32>().unwrap());

    assert!(
        val > 0f32 && val < 100f32,
        "Value of {} must be a decimal between 0 and 100",
        arg
    );
...
fn print_help_and_exit(opts: &[<Options as Example>::OptGroup]) {
    println!("{}", Options::usage("Duplicate Image Finder", opts));
    super::exit();
}
